...
function block_mytermcourses_createsections($newcourse, $newcontext) {
    ...
    $specialcontexts = array(4510 => array('DESCRIPTION DU COURS',
                                           'PLAN DE COURS',
                                           'FICHES TD',
...
function block_mytermcourses_similarcourses($newcourseidnumber) {
	global $DB;
	$html = '';
	$sql = "SELECT id, idnumber, fullname, shortname, visible FROM {course} WHERE idnumber = '$newcourseidnumber' OR idnumber LIKE '$newcourseidnumber+%'";
    ...
    foreach ($availablevet->courses as $availablecourse) {
        ...
        if ($alreadycourses) {
            ...
            $againconfirm = '<div style="text-align:center">'.get_string('reallycreateagain', 'block_mytermcourses')
                .block_mytermcourses_creationform($coursecategory->id,
                        $availablecourse->coursecodeyear, $availablecourse->coursename).'</div>';
...
function block_mytermcourses_choosecategory($upcategory) {
	...
	if ($categories) {
	    ...
	    if ($upcategory->id == 3153) {
 		    echo "<p><span style='font-weight:bold;color:red'>Attention : Les cours sont créés par les gestionnaires administratifs de l'UFR Droit. "
                ."Nous remercions les enseignants de se rapprocher de leur gestionnaire pédagogique pour la création d'un cours.</span></p>";
	    }
